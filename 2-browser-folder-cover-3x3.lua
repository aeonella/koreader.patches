...
local function patchCoverBrowser(plugin)
    ...
    function MosaicMenuItem:_setFolderCover(img)
        local total_w = self.width - 2 * Folder.face.border_size
        local total_h = self.height - 2 * Folder.face.border_size
        
        -- Use the same standard 2:3 book cover aspect ratio (Width:Height)
        local target_aspect_ratio = 2.0 / 3.0
        
        local target = {}
        if total_w / total_h > target_aspect_ratio then
            -- Constrained by height, calculate width
            target.h = total_h
            target.w = math.floor(total_h * target_aspect_ratio)
        else
            -- Constrained by width, calculate height
            target.w = total_w
            target.h = math.floor(total_w / target_aspect_ratio)
        end
        ...
        if img.scale_to_fit then
            ...
            -- IMPORTANT: Even when not scaling to fit, we need to set the bounds
            img_options.width = target.w
            img_options.height = target.h
        end
        ...
        local image_widget = FrameContainer:new {
            ...
            CenterContainer:new {  -- Use CenterContainer for perfect vertical/horizontal alignment
                dimen = { w = target.w, h = target.h }, -- Use the aspect-ratio constrained size
                image,
            },
            overlap_align = "center",
        }
        local directory = self:_getTextBox { w = target.w, h = target.h }
    ...
    function MosaicMenuItem:paintTo(bb, x, y)
        ...
        local fx = x + offset.x
		local vertical_shift = 10
        local fy = y + offset.y  -- REMOVED MANUAL OFFSET
        local fw, fh = dimen.w, dimen.h
        
        -- Separate offsets for top and bottom corners
        local top_corner_offset = -0  -- Adjust this to move top corners (negative = up, positive = down) REMOVED
        local bottom_corner_offset = -0  -- Adjust this to move bottom corners (negative = up, positive = down) REMOVED
